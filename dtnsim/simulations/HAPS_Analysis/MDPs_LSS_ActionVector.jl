...
function contactPlanGatherer()
    # Read the file
    file_path = "/home/benoitcoeugnet/git/dtnsim/dtnsim/simulations/HAPS_Analysis/FilteredContactPlans/contact_plan_7d_node-ids_1LEO-2GS.txt"
...
function transition(s, a)
    ...
    if (s[1] >= 1 && a == "[0,1,0]" && (s[4] in contactPlan[1]))
        # println("Transition from state $s with action $a to HAGS 1")
        ns[1] -= 1
        ns[2] += 1
    end

    if (s[1] >= 1 && a == "[0,0,1]" && (s[4] in contactPlan[2]))
        # println("Transition from state $s with action $a to HAGS 2")
        ns[1] -= 1
        ns[3] += 1
    end

    if (s[1] >= 2 && a == "[0,1,1]" && (s[4] in contactPlan[1]) && (s[4] in contactPlan[2]))
        # println("Transition from state $s with action $a to HAGS 1 and HAGS 2")
        ns[1] -= 2
        ns[2] += 1
        ns[3] += 1
    end
...
generator = CustomStateGenerator(3, 20, maxOfTime)
...
TTF = [parse(Float64, x) for x in TTF]
TTR = [parse(Float64, x) for x in TTR]
println("TTF: ", TTF)
println("TTR: ", TTR)
...
for sn in MCTS.state_nodes(treeAct)
    ...
    best_q = -Inf
    best_action = nothing
    # Iterate over children of the state node to select the best action
    for san in MCTS.children(sn)
        # Only consider candidates with nonzero Q and a considerable visitation count.
        if MCTS.q(san) != 0 && MCTS.n(san) > 100
            q_value = MCTS.q(san)
    ...
    # If a best action was found, store it in the results dictionary.
    if best_action !== nothing
        if best_action == "[0,1,1]"
            # If the best action is "[0,1,1]", write two entries with transformed actions  
            results[state] = ("[0,1,0]", best_q)
            otherState = copy(state)
            otherState[1] -= 1
            otherState[2] += 1
            results[otherState] = ("[0,0,1]", best_q)
        else
            # Otherwise, simply store the best action.
            results[state] = (best_action, best_q)
        end
    end
end
...
# Load existing data by reading the file as a String then parsing.
existing_data = Dict{String, Any}()
if isfile(file_path) && filesize(file_path) > 0
    file_string = read(file_path, String)
    println("Debug: file_string before parsing: ", file_string)
    existing_data = JSON.parse(file_string)
end

println("Existing data :", existing_data)

# Merge the new results into the existing data by simply overwriting any existing value.
for (key, value) in results
    key_str = JSON.json(key)
    existing_data[key_str] = value
end

json_data = JSON.json(existing_data)
