...
generator = CustomStateGenerator(3, 10, maxOfTime)
...
state_size = 5  # Size of your state vector
num_actions = 4  # Number of possible actions

model = Chain(
    Dense(state_size, 128, relu),  # Increased size for better capacity
    Dense(128, 128, relu),
    Dense(128, num_actions)
)  # Move the model to the GPU using CUDA


exploration = EpsGreedyPolicy(mcts_mdp, LinearDecaySchedule(start=1.0, stop=0.01, steps=10000))

solver = DeepQLearningSolver(
    qnetwork = model,
    max_steps = 1000,  # Adjust based on your needs
    exploration_policy = exploration,
    learning_rate = 0.001,  # Adjust based on experimentation
    log_freq = 500,
    recurrence = false,
    double_q = true,
    dueling = true,
...
policy = solve(solver, mcts_mdp)

sim = RolloutSimulator(max_steps=100)
total_reward = simulate(sim, mcts_mdp, policy)
