...
void Fault::initialize()
{
	...
	if ((this->par("enable").boolValue() == true) && eid_ != 0)
	{
		...
		cout << "With " << meanTTF << " initializing fault at " << expValue << " seconds at node " << eid_ <<  "\n";
...
void Fault::handleMessage(cMessage *msg)
{
	if (msg->getKind() == FAULT_START_TIMER)
	{
		...
		cout << "With " << meanTTR << " Recovery at " << simTime() + expValue << " seconds at node " << eid_ << "\n";
	...
	else if (msg->getKind() == FAULT_END_TIMER)
	{
		...
		double expValue = exponential(meanTTF);
		cout << "With " << meanTTF << " Next fault at " << simTime() + expValue <<  " seconds at node " << eid_ <<  "\n";
		scheduleAt(simTime() + expValue, msg);
