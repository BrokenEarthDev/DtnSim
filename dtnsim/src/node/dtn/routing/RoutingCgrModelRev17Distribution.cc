...
RoutingCgrModelRev17Distribution::RoutingCgrModelRev17Distribution(int eid, int nodeNum, SdrModel * sdr, ContactPlan * localContactPlan,
		ContactPlan * globalContactPlan, string routingType, bool printDebug, int numHags, string MeanTTF, string MeanTTR)
	: RoutingDeterministic(eid, sdr, NULL)
{
	...
	numHags_ = numHags;				// number of HAGs

	if (routingType_.find("weighted") != std::string::npos) {
		std::vector<std::string> MeanTTF_;
		std::stringstream s_stream_TTF(MeanTTF);
		while(s_stream_TTF.good()) {
			std::string substr;
			std::getline(s_stream_TTF, substr, '_');
			MeanTTF_.push_back(substr);
		}

		std::vector<std::string> MeanTTR_;
		std::stringstream s_stream_TTR(MeanTTR);
		while(s_stream_TTR.good()) {
			std::string substr;
			std::getline(s_stream_TTR, substr, '_'); 
			MeanTTR_.push_back(substr);
		}

		std::vector<double> cloudCoverage;
		for (int i = 0; i < MeanTTR_.size(); i++) {
			double ttr = std::stod(MeanTTR_[i]);
			double ttf = std::stod(MeanTTF_[i]);
			// std::cout << "ttr: " << ttr << " ttf: " << ttf << endl;
			// std::cout << (ttf / (ttr + ttf)) << endl;
			cloudCoverage.push_back((ttf / (ttr + ttf)*100));
		}
		...
		for (int i = 0; i < cloudCoverage.size(); i++) {
			distribution.push_back(round((cloudCoverage[i] / sum)*100));
		}
		...
		if (diff < 0) {
			// If the sum is less than 1, increment the largest value
			*std::max_element(distribution.begin(), distribution.end()) -= diff;
		} else if (diff > 0) {
			// If the sum is greater than 1, decrement the smallest value
			*std::min_element(distribution.begin(), distribution.end()) -= diff;
		}
		...
		int value = 3;
		distributionVector.assign(100, 0);
		for (int i = 0; i < distribution.size(); i++) {
			int index = 0;
			for (int j = 0; j < distribution[i]; j++) {
				...
				while (distributionVector[index] != 0) {
					index++;
					if (index >= 100) {
						index = 0;
					}
				...
				// std::cout << "distribution[i] " << distribution[i] << "// j " << j << endl;
				// std::cout << "index :" << index << endl;
				distributionVector[index] = value;
				index = index + (100 / distribution[i]);
				// Print distributionVector
				// for (int i = 0; i < distributionVector.size(); i++) {
				// 	std::cout << "Distribution Vector " << i << ": " << distributionVector[i] << endl;
				// }
				// cout << index << endl;
			}
			value = value + 2;
		...
		// cout << distributionVector[13] << endl;
		...
		// for (int i = 0; i < distribution.size(); i++) {
		// 	int value = 3 + (i * 2);
		// 	int number = distribution[i];
		// 	for (int j = 0; j < number; j++) {
		// 		distributionVector.push_back(value);
		// 	}
		// ---------------------------------------------------------------------
...
RoutingCgrModelRev17Distribution::~RoutingCgrModelRev17Distribution() {
...
void RoutingCgrModelRev17Distribution::cgrForward(BundlePkt * bundle) {
	...
	if (routingType_.find("routeListType:oneBestPath") != std::string::npos) {
		...
		if (routeTable_.at(terminusNode).at(0).residualVolume < bundle->getByteLength()) {
			...
			cout << "needRecalculation = true 3" << endl;
			needRecalculation = true;
		}
	...
	if (!routeTable_.at(terminusNode).empty()) {
		...
		if (routingType_.find("distribution:equal") != std::string::npos) {
			...
			for (auto it = routeTable_.at(terminusNode).begin(); it != routeTable_.at(terminusNode).end(); ++it) {
				if (it->hops.size() >= 2 && it->hops[it->hops.size() - 2]->getSourceEid() == 3 + (((bundle->getBundleId()) % numHags_) * 2) && !it->filtered) {
					if (bestRoute == routeTable_.at(terminusNode).end() || it->arrivalTime < bestRoute->arrivalTime) {
						bestRoute = it;
					}
				}
			}
		...
		} else if (routingType_.find("distribution:weighted") != std::string::npos) {
			// Print all routes in the routeTable
			for (const auto& route : routeTable_.at(terminusNode)) {
				cout << "Route: ";
				for (const auto& hop : route.hops) {
					cout << hop->getSourceEid() << " -> ";
				}
				cout << terminusNode << endl;
			}
			...
			cout << "BID : " << bundle->getBundleId() <<  ", FT : " << bestRoute->fromTime << ", TT : " << bestRoute->toTime << ", AT : " << bestRoute->arrivalTime << ", NH : " << bestRoute->nextHop << " // Chosen route: ";
			for (const auto& hop : bestRoute->hops) {
				cout << hop->getSourceEid() << " -> ";
			}
			cout << terminusNode << endl;
		...
		} else if (routingType_.find("distribution:opportunistic") != std::string::npos) {
			cout << "Opportunistic" << endl;
...
void RoutingCgrModelRev17Distribution::cgrEnqueue(BundlePkt * bundle, CgrRoute *bestRoute) {
	...
	if (bestRoute->nextHop != NO_ROUTE_FOUND && !bestRoute->filtered) {
		...
		if (routingType_.find("contactPlan:local") != std::string::npos
			...
			if (routingType_.find("volumeAware:allContacts") != std::string::npos) {
				...
				for (vector<Contact *>::iterator hop = bestRoute->hops.begin(); hop != bestRoute->hops.end(); ++hop) {
					...
					// This should never happen. We'll temporarily leave
		...
		if (routingType_.find("sdrModel:perContact") != std::string::npos) {
			...
			cout << "queuing bundle in node " << bestRoute->hops.at(0)->getDestinationEid() << "(" << bestRoute->hops.at(0)->getId() << ")" << endl;
...
void RoutingCgrModelRev17Distribution::findNextBestRoute(vector<int> suppressedContactIds, int terminusNode, CgrRoute * route) {
...
void RoutingCgrModelRev17Distribution::clearRouteTable() {
...
void RoutingCgrModelRev17Distribution::printRouteTable(int terminusNode) {
...
bool RoutingCgrModelRev17Distribution::compareRoutes(CgrRoute i, CgrRoute j) {
...
void RoutingCgrModelRev17Distribution::checkRoutingTypeString() {
...
int RoutingCgrModelRev17Distribution::getDijkstraCalls() {
	return dijkstraCalls;
}

int RoutingCgrModelRev17Distribution::getDijkstraLoops() {
	return dijkstraLoops;
}

int RoutingCgrModelRev17Distribution::getRouteTableEntriesCreated() {
	return tableEntriesCreated;
}

int RoutingCgrModelRev17Distribution::getRouteTableEntriesExplored() {
